if (document.querySelector('.slider__body')) {
	new Swiper('.slider__body', {
		...
		// Dotts
		pagination: {
			el: '.slider__paginations',
			clickable: true,
		},
		// Arrows
		navigation: {
			nextEl: '.slider__arrow.slider__arrow_next',
			prevEl: '.slider__arrow.slider__arrow_prev',
		},
	});
}

const slider = document.querySelector('.slider-card__swiper');
const sliderNav = document.querySelector('.slider-card__nav');
if (sliderNav && slider) {
...
//=================
// Filtr isotope
if (document.querySelector('.filtr__items')) {
	var iso = new Isotope('.filtr__items', {
		// options
		itemSelector: '.filtr__item',
		masonry: {
			columnWidth: 368,
			gutter: 30,
			fitWidth: true
	...
	var filtersElem = document.querySelector('.filtr__controls');

	filtersElem.addEventListener('click', function (event) {
		// only work with buttons
		if (!matchesSelector(event.target, '.filtr__button')) {
			return;
		}
	...
	let controlButtons = document.querySelectorAll('.filtr__button');
	for (i = 0; i < controlButtons.length; i++) {
		let controlButton = controlButtons[i];
		controlButton.addEventListener('click', () => {
			for (k = 0; k < controlButtons.length; k++) {
				if (!matchesSelector(event.target, '.filtr__button')) {
					return;
				}
				if (controlButtons[k].classList.contains('is-checked')) {
					controlButtons[k].classList.remove('is-checked')
				}
			}
			controlButton.classList.add('is-checked');
		});
	}
}
...
const input_icon = document.querySelector('.search-form__icon');
input_icon.addEventListener('click', () => {
	let search = document.querySelector('.search-form__item');
...
// Скрываем input, если кликнули не по нему
document.addEventListener('click', (e) => {
	if (document.querySelector('.search-form__item').classList.contains('_active')) {
		if (!e.target.closest('.search-form__icon') && !e.target.closest('.search-form__item')) {
...
if (document.querySelector('.card__btn')) {
	// Активирую событие клика на кнопку в блоке портфолио
	let btn = document.querySelector('.card__btn');
	let showSocials = document.querySelector('.card__socials');
	btn.addEventListener('click', () => {
		if (showSocials.classList.contains('_active')) {
			showSocials.classList.remove('_active');
		} else {
			showSocials.classList.add('_active');
		}
	})
	// Убираю класс active, если кликнули не по нему
	document.addEventListener('click', (e) => {
		if (showSocials.classList.contains('_active')) {
			if (!e.target.closest('.card__btn') && !e.target.closest('.card__socials')) {
				showSocials.classList.remove('_active');
			}
		}
	})
	...
	if (scr_items.length > 0) {
		for (let index = 0; index < scr_items.length; index++) {
			...
			if ((src_value > scr_item_offset - scr_item_point) && src_value < (scr_item_offset + scr_item_height)) {
				...
				if (!scr_item.classList.contains('_anim-no-hide')) {
